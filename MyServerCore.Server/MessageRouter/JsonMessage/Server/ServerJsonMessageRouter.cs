...
namespace MyServerCore.Server.MessageRouter.JsonMessage.Server
{
    ...
    /// 
    /// </summary>
    public class ServerJsonMessageRouter : Singleton<ServerJsonMessageRouter>
    {
        /// <summary>
        /// 最大线程数
        /// </summary>
        ...
        /// 
        /// </summary>
        private Queue<ServerJsonMessage> messageQueue = new Queue<ServerJsonMessage>();

        /// <summary>
        /// 消息处理字典
        /// </summary>
        private ConcurrentDictionary<string, Action<TcpSession, object>> currentMessageHandlers =  new ConcurrentDictionary<string, Action<TcpSession, object>>();
        /// <summary>
        /// 
        /// </summary>
        ...
        public void AddMessageDataToQueue(TcpSession tcpClient, string json)
        {
            ...
            lock (messageQueue)
            {
                messageQueue.Enqueue(new ServerJsonMessage() { jsonMessage = message, tcpSession = tcpClient });
            }
        ...
        private bool IsJson(string json)
        {
            ...
            // 判断是否以 '{' 或 '[' 开头
            return trimmedJson.StartsWith("{") || trimmedJson.StartsWith("[");
        }
        ...
        public void AddNewMessageHandlersToDictionary<T>(Action<TcpSession, object> action) where T : class
        {
        ...
        private void MessageWork(object? state)
        {
            try
            {
                ...
                while (isRunning)
                {
                    ...
                    ExcuteLoopMessage(mes.jsonMessage, mes.tcpSession);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="tcpClient"></param>
        private void ExcuteLoopMessage(CJsonMessage jsonMessage, TcpSession tcpClient)
        {
